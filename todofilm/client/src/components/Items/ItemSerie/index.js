...
function ItemSerie({ id, poster, title }) {
    ...
    const removeMovie = (id) => {
        axios.delete( `/api/user/${userId}/wishlistSerie/${id}`)
            .then(function (reponse) {
                ...
                wishlistContext.removeSerie(id)
            })
            .catch(function (erreur) {
                console.log(erreur);
            })
        ...
        <>
            {
                ...
                : <div className={`bloc-item-film`}>
                    <Link className='link-poster' to={`/serie/${id}`}>
                        {
                            poster
                            ? <img key={id} className='bloc-item-poster' src={`https://image.tmdb.org/t/p/w500${poster}`} alt={`Affiche ${title}`} />
                            : <img key={id} className='bloc-item-poster' src={defaultMovie} alt="image de film par défaut" />
                        }
                    </Link>
                    ...
                    <div className="bloc-item-film-title">
                        ...
                        <Link className='link-title' to={`/serie/${id}`}>
                            <p className='title-item-film'>{title}</p>
                        </Link>
                    </div>
                </div>
            }
